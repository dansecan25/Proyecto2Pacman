...
/*void SocketServer() {
    ...
    // Procesar mensajes entrantes en un bucle
    while (true) {
        char buffer[1024] = {0};
    ...
    // Cerrar el socket solo cuando se sale del bucle
    if (close(server_fd) == -1) {
        std::cerr << "Error al cerrar el socket del servidor: " << std::strerror(errno) << std::endl;
        return;
    }
    if (close(new_socket) == -1) {
        std::cerr << "Error al cerrar el socket del cliente: " << std::strerror(errno) << std::endl;
        return;
    }
}
